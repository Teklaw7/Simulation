...
namespace glhelper
{
  ...
  GLuint create_program(const std::string& cs_content)
  {
    GLuint cs_id = compile_shader(cs_content.c_str(),GL_COMPUTE_SHADER);
  ...
  {
    return create_program(read_file(cs_file));
  }
